...
Class usuarios extends Controller {
	...
	private $usuarios;			//	ATRIBUTO PARA INSTANCIAR O MODELO DE DADOS
	...
	public function index_action() {	//	LISTAGEM DE TODOS OS REGISTROS OU DE ACORDO COM O FILTRO DE PESQUISA
		...
		$this->LimpaFormulario("usuarios");	//	limpa o formulario e sessao de erros...
		...
		if ( isset($_POST['fw_pesquisar']) and !empty($_POST['fw_pesquisar']) ) {
			...
			$this->datas['lista_registros'] = $this->usuarios->listausuarios(" `{$campo}` " . $op);	
			$totreg = count($this->datas['lista_registros']);	//	Total de registros com filtro

			$this->datas['lista_registros'] = $this->usuarios->listausuarios(" `{$campo}` " . $op,$numreg,($pag-1)*$numreg);	//	Pesquisa com condiзгo
		} else {
			$this->datas['lista_registros'] = $this->usuarios->listausuarios();
			$totreg = count($this->datas['lista_registros']); // total de registros 
		
			$this->datas['lista_registros'] = $this->usuarios->listausuarios(null,$numreg,($pag-1)*$numreg);						//	Passa o resultado para o vetor $datas
		}
	...
	public function consultar() {			//	CONSULTANDO UM REGISTRO!!!
		...
		$this->datas['show_registro'] = $this->verificaRegistro($id, $this->usuarios->usuariosShow("id=".$id));	//	VERIFICA SE O REGISTRO EXISTE!!!
	...
	public function excluir() {						//	EXCLUINDO REGISTRO!!!
		...
		if (count($this->emprestimos->emprestimosShow("usuario_id = ".$id)) != 0) {
			$this->view('aviso_excluir', $this->datas);
			return;
		}
		
		if ($confirma == "sim") {
			$this->usuarios->delete('id ='.$id);
	...
	public function incluir() {						//	INCLUINDO UM REGISTRO!!!
		...
		if ($confirma == "sim") {
			$form = array();
			$form = $this->retorna_formulario($this->usuarios->_tabela);	//	Lendo dados do formulario
			
			if (($form['bloqueado']) != "") {	//	Campo nao setado й sempre vazio... TIPO CHECKBOX...
				$form['bloqueado'] = "S";
			} else {
				$form['bloqueado'] = "N";
			}			
			...
			$this->datas['erros'] =  $this->usuarios->ValidaDados($form,$campos);	//	validacao de dados...	
			
			if (count($this->datas['erros']) == 0) {		//	 se existem erros volta para o formulario
				$this->LimpaFormulario("usuarios");	//	limpa o formulario e sessao de erros...					
				
				$this->usuarios->insert($form);
				$this->redirecionar_pagina_inicial();	
			}else{
	...
	//	ALTERANDO REGISTRO!!!
		...
		if ($confirma == "sim") {
			...
			if (count($this->datas['erros']) == 0) {		//	 se existem erros volta para o formulario
				$this->LimpaFormulario("usuarios");		//	limpa o formulario e sessao de erros...		
				$this->usuarios->update( $form, "id=".$id) ;
				$this->redirecionar_pagina_inicial();	
			}else{
				$redireciona = new RedirectorLib();		
